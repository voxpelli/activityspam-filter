...
import { NoSuchThingError } from './simple-databank.js';
...
export class SpamFilter {
  ...
  #db;
  /** @type {import('./tokenizer.js').Tokenizer} */
  #tokenizer;

  /**
   * @param {import('./simple-databank.js').SimpleDatabank} db
   * @param {import('./tokenizer.js').TokenizerOptions} [options]
   */
  ...
  async #updateTokenCounts (cat, tokens, dir) {
    ...
    const catTotal = await (
      dir === UP
        ? this.#db.incr(catTotalKey, '1')
        : this.#db.decr(catTotalKey, '1')
    );
  ...
  /**
   * @param {import('type-fest').JsonObject | import('type-fest').JsonArray} obj
   * @returns {Promise<SpamFilterDecision>}
   */
  async test (obj) {
    const start = Date.now();
    const tokens = this.#tokenizer.tokenize(obj);
